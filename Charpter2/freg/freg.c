...
static ssize_t freg_read(struct file* filp, char __user* buf, size_t count, loff_t* f_pos) {
    ...
    if(down_interruptible(&(dev_>sem))) {
        return -ERESTARTSYS;
    }
...
static ssize_t freg_proc_write(struct file* filp, const char __user* buff, unsigned long len, void* data) {
    ...
    page = (char*)__get_free_page(GEP_KERNEL);
    if(!page) {
...
static int __init freg_init(void) {
    ...
    freg_dev = kmalloc(sizeof(struct fake_reg_dev), GEP_KERNEL);
    if(!freg_dev) {
        err = -ENOMEM;
    ...
    if(IS_ERR(temp)) {
        err = PTR_ERR(temp);
        printk(KERN_ALERT"Failed to create freq device.\n");
        goto destroy_class;
    }
...
destroy_device:
    device_destory(freg_class, dev);
destroy_class:
    device_destory(freg_class);
destroy_cdev:
...
static void __exit freg_exit(void) {
    ...
    if(freg_class) {
        device_destory(freg_class, MKDEV(freg_major, freg_minor));
        class_destory(freg_class);
    }
...
MODULE_LICENSE("GPL");
MODUEL_DESCRIPTION("Fake Register Driver");
